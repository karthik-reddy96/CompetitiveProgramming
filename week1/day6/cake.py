...
import unittest
def value(cake_tuples, weight_capacity, n):    
    array=[0]*(weight_capacity+1)
    for i in range(weight_capacity + 1):
        for j in range(n):
            if (cake_tuples[j][0] <= i):
                array[i] = max(array[i], array[i - cake_tuples[j][0]] + cake_tuples[j][1])

    return array[weight_capacity]
...
def max_duffel_bag_value(cake_tuples, weight_capacity):
    for cake in cake_tuples:
        if (cake[0] == 0 and cake[1] != 0):
            return float('inf')
    n = len(cake_tuples)
    return value(cake_tuples, weight_capacity, n)
    return -1
